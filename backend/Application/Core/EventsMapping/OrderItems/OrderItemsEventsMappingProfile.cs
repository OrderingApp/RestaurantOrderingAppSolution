...
public class OrderItemsEventsMappingProfile : Profile
{
    public OrderItemsEventsMappingProfile()
    {
        ...
        CreateMap<OrderItem, OrderItemUpdatedEvent>()
            ...
            .ForMember(dest => dest.ExtraIngredientIds, opt => opt.MapFrom(src => src.ExtraIngredients.Select(e => e.Id).ToList()))
            .ForMember(dest => dest.RemovedIngredientIds, opt => opt.MapFrom(src => src.RemovedIngredients.Select(r => r.Id).ToList()));

        CreateMap<OrderItem, OrderItemDeletedEvent>()
            .ForMember(dest => dest.OrderItemId, opt => opt.MapFrom(src => src.Id));

        CreateMap<(OrderItem orderItem, OrderItemStatus previousStatus), OrderItemStatusUpdatedEvent>()
