...
public class OrderUpdateTypeDtoValidator : AbstractValidator<OrderUpdateTypeDto>
{
    ...
    {
        RuleFor(x => x.TableId)
            .NotEmpty().WithMessage("TableId must be a valid GUID when provided.")
            .When(x => x.NewOrderType == OrderType.DineIn);

        RuleFor(x => x.PhoneNumber)
            .NotEmpty().WithMessage("PhoneNumber is required for Takeaway and Delivery orders.")
            .Matches(@"^\d{9,15}$").WithMessage("PhoneNumber must be between 9 and 15 digits.")
            .When(x => x.NewOrderType == OrderType.Takeaway || x.NewOrderType == OrderType.Delivery);
        ...
        RuleFor(x => x.NewOrderType)
            .IsInEnum().WithMessage("Invalid order type specified.");
    }
}