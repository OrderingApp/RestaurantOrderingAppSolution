...
public class SubCategoryService(RestaurantOrderingContext orderingContext, IEventHandlerService eventHandlerService, IMapper mapper) : ISubCategoryService
{
    public async Task<ResultDto<SubCategoryReadDto>> CreateSubCategory(SubCategoryCreateDto subCategoryCreateDto)
    {
        try
        {
            var menuCategoryExists = await orderingContext.MenuCategories
            .AnyAsync(mc => mc.Id == subCategoryCreateDto.MenuCategoryId && !mc.IsDeleted);

            if (!menuCategoryExists)
                return ResultDto<SubCategoryReadDto>
                    .Failure("Invalid MenuCategoryId. The referenced menu category does not exist.", HttpStatusCode.BadRequest);
            ...
            return ResultDto<SubCategoryReadDto>
                .Success(createdSubCategory, HttpStatusCode.Created);
        }
        catch (Exception ex)
        {
            return ResultDto<SubCategoryReadDto>
    ...
    public async Task<ResultDto<SubCategoryReadDto>> GetSubCategory(Guid id)
    {
        try
        {
            var subCategory = await orderingContext.SubCategories
                .AsNoTracking()
                .FirstOrDefaultAsync(sc => sc.Id == id);

            if (subCategory == null)
                return ResultDto<SubCategoryReadDto>
                    .Failure("SubCategory not found.", HttpStatusCode.NotFound);

            var subCategoryDto = mapper.Map<SubCategoryReadDto>(subCategory);

            return ResultDto<SubCategoryReadDto>
                .Success(subCategoryDto, HttpStatusCode.OK);
        }
        catch (Exception ex)
        {
            return ResultDto<SubCategoryReadDto>
    ...
    public async Task<ResultDto<List<SubCategoryReadDto>>> GetSubCategories()
    {
        try
        {
            var subCategories = await orderingContext.SubCategories
            ...
            return ResultDto<List<SubCategoryReadDto>>
                .Success(subCategoryDtos, HttpStatusCode.OK);
        }
        catch (Exception ex)
        {
            return ResultDto<List<SubCategoryReadDto>>
    ...
    public async Task<ResultDto<SubCategoryReadDto>> UpdateSubCategory(Guid id, SubCategoryUpdateDto subCategoryUpdateDto)
    {
        try
        {
            ...
            if (subCategoryToUpdate == null)
                return ResultDto<SubCategoryReadDto>
                    .Failure("subCategory not found or has been deleted.", HttpStatusCode.NotFound);

            var menuCategoryExists = await orderingContext.MenuCategories
            .AnyAsync(mc => mc.Id == subCategoryUpdateDto.MenuCategoryId && !mc.IsDeleted);
            ...
            return ResultDto<SubCategoryReadDto>
                .Success(updatedSubCategory, HttpStatusCode.OK);
        }
        catch (Exception ex)
        {
            return ResultDto<SubCategoryReadDto>
    ...
    public async Task<ResultDto<bool>> DeleteSubCategory(Guid id)
    {
        try
        {
            ...
            if (subCategory.IsDeleted)
                return ResultDto<bool>
                    .Failure("subCategory has already been deleted.", HttpStatusCode.BadRequest);
